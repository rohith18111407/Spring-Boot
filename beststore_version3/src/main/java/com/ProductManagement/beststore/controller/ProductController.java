...
//	}
	...
	@GetMapping("/admin/jpa/products/search")
    public String searchProducts(@RequestParam("productName") String productName, Model model) {
        try {
            List<Product> products;

            if (productName != null && !productName.isEmpty()) {
                // Perform search by product name in repository
                products = productRepository.findByNameIgnoreCaseContaining(productName);
            } else {
                // Handle case where no search parameter is provided
                products = productRepository.findAll();
            }

            model.addAttribute("products", products);
            return "products/adminHomePage"; 
        } catch (Exception ex) {
            System.out.println("Exception: " + ex.getMessage());
            return "products/adminHomePage"; 
        }
...
//	            Page<Product> productPage = repository.findAll(PageRequest.of(page, pageSize));
//
//	            // Extract product list from page object
...
//		            List<Product> filteredProjects = productRepository.findByNameIgnoreCaseContaining(searchname);
...
//		            productPage = new PageImpl<>(filteredProjects.subList(start, end), PageRequest.of(page, pageSize), filteredProjects.size());
//		        } else {
//		            productPage = productService.getPaginatedProjects(PageRequest.of(page, pageSize));
//		        }
